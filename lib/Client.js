...
var Client = module.exports = function Client(options) {
    ...
    this._autoConnectOnWrite = this.options.autoConnectOnWrite
...
/**
 * Get the statistics for the current client
 * include message counts, buffer size, etc.
 * 
...
Client.prototype.getStats = function() {
    return { host: this.connection.options.host,
...
Client.prototype.fetchTopic = function(args) {
	var request = null;
	if (_.isObject(args)) {
		request	 = _.defaults(args, this.fetch_defaults);
	} else if (_.isString(args)) {
		request	 = _.defaults({ name: args }, this.fetch_defaults);
	} else {
...
Client.prototype.fetchOffsets = function(args) {
	var request = null;
	if (_.isObject(args)) {
		request	 = _.defaults(args, this.offset_defaults);
	} else if (_.isString(args)) {
		request	 = _.defaults({ name: args }, this.offset_defaults);
	} else {
		throw new Error('Fetch offers invalid parameter, must be name or complex object');
	}
    this._pushRequest({request: request})
    return this
}

Client.prototype.send = function(args, messages, callback) {
	var request = null;
	if (_.isObject(args)) {
		...
		throw new Error('Send invalid parameter, must be name or complex object');
	}
